...
3. The fasta files are blasted against the the NCBI nucleotide blast databases using blastn. 
	* blastn parameters can be altered in the /crux_release_V1_db/crux_vars.sh file
4. The blast results are de-replicated by NCBI accession version number and converted into fasta format.
5. entrez-qiime is then used to determine taxonomy for each read based on NCBI version accession number. 
6. ** pending **
...
7. Reduce the dataset to read clusters with 99% identity using Usearch, and assign taxonomy at 97% using uclust.  Both are implemented in Qiime
